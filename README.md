...
项目已完成首个离线指挥链技术切片：精确单选与框选、五类单位职责隔离、三类建筑选择与生产、双方独立矿区与真实矿车往返、双方矿车采矿中自卫、工程车施工/维修、动态建筑占格、对角最短路径、远程目标追击与重新寻路、单位与建筑的统一弹丸战斗、纯手动胜利链、阵营知识、玩家接管与归队、三项高层委托，以及敌方经济/扩张/侦察/争矿/扩军/袭击/撤退/回防阶段机均已接入 10 Hz 权威模拟。`Q` 统一承担集火和移动攻击，目标模式会显示已选作战单位的真实射程与目标指示；工业委托可与战场委托并行，新生产的兼容单位会加入开放任务。施工模式提供吸附格点、占格范围、接近区和合法性预览。我方工业主管与战场将领支持建议、协助、委托和自主四级授权；敌方提供简单、普通、困难、专家四档公平信息 AI，并按难度调整决策频率、威胁反应、目标/路线评分、有限追击、撤退阈值和基于已观察兵种的反制生产。敌方仍复用相同定义、命令、寻路、弹丸和伤害规则。左上金币条、左侧实时工作流、左下战术小地图、底部选择情报/战略/工程/生产分区指挥台、建筑/单位造价、任务路线可视化和中英双语 UI 同步可用；开发诊断层默认隐藏，可按 `F3` 切换。玩家基地位于地图左上角，敌方基地位于右下角。项目自有 runner 当前通过 13 个测试套件和 6 分钟权威切片 smoke，Windows debug export 已在具备同版本模板的环境验证。

这仍不是完整 15—20 分钟 MVP。下一阶段重点是 5—8 分钟垂直切片人工试玩与数值调优、生产建筑职责与队列、更多战术命令和编队、任务并发策略、CI、操作反馈和正式美术。详细状态见[当前进展与下一步](docs/STATUS_AND_NEXT_STEPS.md)。
