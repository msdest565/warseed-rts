...
最新一轮稳定性与自主 AI 升级将渲染目标设为 120 FPS（垂直同步显示器稳定按刷新率运行，最低验收目标为 60 FPS），而权威模拟继续保持 10 Hz。表现层现在每个权威快照只同步一次代理和 HUD；存活但失去视野的敌军显示为半透明情报残影，只有确认摧毁才显示死亡残骸。自主侦察车发现近距敌军后会清除交战意图、向可达安全区撤离并在威胁解除后恢复侦察，同时通过顶部敌情条和小地图信号提醒玩家。底部参谋部可在“未接管/手动”与“均衡接管、经济优先、基地固守、主动进攻”四种接管方针间切换；选择接管方针会统一开启工业主管与战场将领的自主权限，并让参谋部持续仲裁生产、侦察、补员和攻防任务。自主经济以 2 辆矿车为基础编制（经济优先为 3 辆），战斗预备队目标为 1 辆侦察车、3 辆突击车和 2 辆导弹车，常规扩军保留 400 矿石应急储备，并给玩家保留生产队列槽。自主攻击失去目标后会前往最后确认位置搜索，重新发现后继续攻击。工作流会显示当前方针、决策以及待付、储备和可用预算。
