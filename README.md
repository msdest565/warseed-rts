...
项目已完成首个离线指挥链技术切片：精确单选与框选、五类单位职责隔离、三类建筑选择与生产、双方独立矿区与真实矿车往返、双方矿车采矿中自卫、工程车施工/维修、动态建筑占格、对角最短路径、远程目标追击与重新寻路、单位与建筑的统一弹丸战斗、纯手动胜利链、阵营知识、玩家接管与归队、三项高层委托，以及敌方经济/扩张/侦察/争矿/扩军/袭击/撤退/回防阶段机均已接入 10 Hz 权威模拟。`Q` 统一承担集火和移动攻击，目标模式会显示已选作战单位的真实射程与目标指示；工业委托可与战场委托并行，新生产的兼容单位会加入开放任务。施工模式提供吸附格点、占格范围、接近区和合法性预览。敌方会补充损失的经济单位、维修受损建筑、建设支援站，并根据基地威胁、部队生命和兵种缺口调整回防、撤退和扩军；敌我复用同一套定义、命令、寻路、弹丸和伤害规则。左上金币条、左侧实时工作流、左下战术小地图、底部选择情报/战略/工程/生产分区指挥台、建筑/单位造价、任务路线可视化和中英双语 UI 同步可用；开发诊断层默认隐藏，可按 `F3` 切换。玩家基地位于地图左上角，敌方基地位于右下角。项目自有 runner 当前通过 12 个测试套件和 6 分钟权威切片 smoke，Windows debug export 已在具备同版本模板的环境验证。
