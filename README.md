...
最新一轮稳定性与自主 AI 升级将渲染目标设为 120 FPS（垂直同步显示器稳定按刷新率运行，最低验收目标为 60 FPS），而权威模拟继续保持 10 Hz。表现层现在每个权威快照只同步一次代理和 HUD；失去视野的敌军显示为固定的最后目击标记，重新发现时不再从隐藏真值插值。自主侦察车发现近距敌军后会清除交战意图、向可达安全区撤离并在威胁解除后恢复侦察，同时通过顶部敌情条和小地图信号提醒玩家。新增规则驱动参谋部统筹工业与战场主管：自主经济以 2 辆矿车为基础编制，战斗预备队目标为 1 辆侦察车、3 辆突击车和 2 辆导弹车，并保留 400 矿石应急储备；当前决策会显示在工作流面板中。
