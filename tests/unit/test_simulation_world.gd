...
func _test_grey_ridge_locked_enemy_reactions_use_faction_knowledge(failures: Array[String]) -> void:
	...
	_expect(feint_world.enemy_reaction_log.filter(func(entry: String) -> bool: return entry.contains("action=FEINT_REDIRECT_CENTRAL")).size() == 1 and feint_world.enemy_reaction_log[-1].contains("source=LOCKED_PLAN_TIMELINE") and feint_world.enemy_reaction_log[-1].contains("action=FEINT_REDIRECT_CENTRAL"), "the scheduled feint redirect should have an auditable non-observation source (log=%s)" % [feint_world.enemy_reaction_log], failures)
