...
1. 解析、静态检查和内容校验；
2. 受影响单元/纯模拟测试；
3. `tests/test_runner.gd` 全套回归（当前18套，以实际入口为准）；
4. 受影响关卡 smoke；
...
验收复用：同一受验源码与测试哈希、环境和命令已有完整日志时，主代理核对原始证据即可，不要求每位代理各跑一次。最终完整发布门已包含的回归不在其前后无变化地重复执行。源码/测试/内容或环境变化只使相关证据失效；纯文档变化不要求重跑游戏。外部文本模型不能执行本地测试，其返回的PASS必须由本地执行补齐。
...
| typed Resource/Loader | 数据单测、引用/范围/重复 ID 校验、全套回归（当前18套） | 受影响内容全部重新加载 |
| 命令/校验 | 合法与拒绝路径、队列时序、快照不可变 | UI 与 Agent 使用同一命令的集成测试 |
| 权威系统/tick 顺序 | 纯模拟、固定种子、同 tick 边界、全套回归（当前18套） | 四关 smoke；必要时完整结局矩阵 |
