...
- 新敌情首次进入视野时触发顶部双语警报和小地图扩散信号，同一目标带 5 秒冷却；失去视野的敌方单位/建筑仅显示静态、半透明的最后目击标记，不再表现为实时目标；
- 新增规则驱动参谋部，协调工业主管与战场将领的自主生产：基础矿车编制上限 2，战斗预备队目标为 1 侦察、3 突击、2 导弹，战斗扩军保留 400 矿石应急储备并给玩家生产队列留出容量；战场将领通过合法 ProduceUnitCommand 生产战斗车辆，参谋部决策显示在工作流面板；
...
覆盖：命令管线、固定 tick、对角最短路径/导航/编队、观测指标、typed data、地图、精确单选/框选/局部编队命令、脱队成员边界、新生产单位与建筑选择、真实生产突击车的单独防守与委托增援、五类单位生产与可负担状态、职责过滤、Q 集火/单体移动攻击/射程预览、编队首 tick 响应与不同射程独立开火、远程追击与移动目标重新寻路、双方矿车对称自卫、目标式采矿/防守/侦察与敌情计数、可达前沿侦察、侦察接敌规避且不攻击、侦察/防守并行、远端接敌反击、基地紧急防守、任务显式替换、600 tick 自主任务所有权与临时编队回收、敌方实际伤害、残骸超时清理、单位与建筑弹丸战斗、手动胜利、建筑施工/占格/完工/维修、真实矿车装卸往返、对角基地布局、经济与胜负、阵营知识、敌方 last-seen 边界、过期接触表现标记、同 tick 表现代理不重复同步、小地图敌情信号、60 FPS 最低项目目标、参谋部矿车上限/合成部队生产/应急资源储备、敌方八阶段经济与作战循环、四档难度参数、反应时间差异、目标/路线评分、观察后兵种反制、有限追击、我方四级权限门禁/增援差异/建议状态/自主任务、经济损失补员/支援站扩张、四项高层任务、接管/归队、工程 UI 命令、右键建筑攻击、中英文即时刷新、完整切片和主场景 UI 控件。
...
- 工业、侦察和一个主力作战任务已经可以并行并按单位仲裁冲突；尚未支持多个独立主力战斗编队和任意任务图；
- 参谋部已经提供基础编制目标、单笔应急资源储备和全局生产节流，但尚未实现跨建筑承诺账本、玩家生产优先级、科技前置或根据敌方长期画像动态调整编制；
...
4. 将现有参谋部基础储备扩展为跨建筑承诺账本、玩家优先级和可取消的动态生产计划，并把敌方已观察兵种、矿区寿命和战损率纳入编制调整；
5. 在现有工业/侦察/主力作战并行基础上，引入多个独立战斗编队、战力评估、任务承诺时间和任意任务图；
