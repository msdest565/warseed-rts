...
- Debug HUD 默认隐藏，按 `F3` 切换，并显示本地化敌方阶段，便于观察、试玩和节奏调优；
...
WARSEED vertical slice: ticks=3600 phases=[0,1,2,3,4,5,6,7,...] damage=75 destroyed=6 player_ore=1200 enemy_ore=560 enemy_gold=1015
```

覆盖：命令管线、固定 tick、对角最短路径/导航/编队、观测指标、typed data、地图、精确单选/框选/局部编队命令、脱队成员边界、新生产单位与建筑选择、五类单位生产与可负担状态、职责过滤、Q 集火/单体移动攻击/射程预览、远程追击与移动目标重新寻路、双方矿车对称自卫、目标式采矿/防守、敌方实际伤害、残骸超时清理、单位与建筑弹丸战斗、手动胜利、建筑施工/占格/完工/维修、真实矿车装卸往返、对角基地布局、经济与胜负、阵营知识、敌方 last-seen 边界、敌方八阶段经济与作战循环、经济损失补员/支援站扩张、三项高层任务、接管/归队、工程 UI 命令、右键建筑攻击、中英文即时刷新、完整切片和主场景 UI 控件。

本轮还通过：

- headless editor import；
- 主场景 headless smoke；
- 1280x720 中英文实际渲染截图检查，确认金币条、左侧工作流、左下小地图和底部分区指挥台无重叠，建筑/生产价格文本完整，Q 模式三种射程环与红色集火指示正常；此前阶段亦已检查 1920x1080、2560x1080；
...
- 敌方 AI 会合法经营和战斗，但仍是单阶段规则机；没有难度档、策略画像、路线风险评估、反侦察或针对玩家兵种的长期克制规划；
