...
底部工程/生产按钮显示这些造价和生产耗时，并依据己方当前矿石与建筑忙碌状态实时禁用；最终平衡仍需人工试玩。
...
- 开放任务会动态吸收尚未被玩家直接命令的新生产兼容单位：矿车领取现有采矿任务，作战单位加入当前战斗编队；
...
- 敌方 Easy/Normal/Hard/Expert 四档配置已数据化，实际控制战略/战术决策间隔、反应延迟、接触记忆、评分噪声、路线威胁权重、集火、攻势门槛、有限追击和撤退滞回；
...
- 我方工业主管与战场将领支持 ADVISORY、ASSISTED、DELEGATED、AUTONOMOUS 四档权限；建议档从命令层拒绝执行，自主档可按合法阵营信息创建基础发展或战斗任务，权限降低会清理未执行命令并暂停越权任务；
- 离线运行，不接入 LLM 或联网服务。
...
WARSEED vertical slice: ticks=3600 phases=[0,1,2,3,4,5,6,7,4,5,6,7,4,5,6,7,4,5] damage=38 destroyed=5 player_ore=1200 enemy_ore=560 enemy_gold=1015
```

覆盖：命令管线、固定 tick、对角最短路径/导航/编队、观测指标、typed data、地图、精确单选/框选/局部编队命令、脱队成员边界、新生产单位与建筑选择、五类单位生产与可负担状态、职责过滤、Q 集火/单体移动攻击/射程预览、远程追击与移动目标重新寻路、双方矿车对称自卫、目标式采矿/防守、敌方实际伤害、残骸超时清理、单位与建筑弹丸战斗、手动胜利、建筑施工/占格/完工/维修、真实矿车装卸往返、对角基地布局、经济与胜负、阵营知识、敌方 last-seen 边界、敌方八阶段经济与作战循环、四档难度参数、反应时间差异、目标/路线评分、观察后兵种反制、有限追击、我方权限门禁与自主任务、经济损失补员/支援站扩张、三项高层任务、接管/归队、工程 UI 命令、右键建筑攻击、中英文即时刷新、完整切片和主场景 UI 控件。
...
- 生产仍是每座建筑单槽位，没有排队、取消退款、集结点编辑和按建筑类型限制单位目录；当前兵工厂可以生产全部五类单位；
...
2. 实现建筑生产目录、可排队生产、取消/退款、集结点和资源预留优先级，使指挥中心负责经济单位、兵工厂负责作战单位；
