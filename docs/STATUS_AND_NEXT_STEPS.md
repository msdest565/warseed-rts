...
- 规则驱动参谋部是全面接管时主动生产的唯一规划者：按经济恢复、基地紧急防御、基础发展、合成部队、稳态的优先级仲裁工业主管与战场将领；统一计算待付生产成本、400 矿石应急储备与常规可用额，给玩家保留每座生产建筑最后一个队列槽；基础矿车编制上限 2，战斗预备队目标为 1 侦察、3 突击、2 导弹，并按已观察敌方构成有限增加反制编制；决策和预算均显示在工作流面板；
...
- 参谋部已经提供跨生产建筑的单位与待付成本承诺统计、玩家队列槽优先级、应急储备和基于当前已观察兵种的有限编制调整；尚未覆盖建筑施工/维修承诺、可取消的多步生产计划、科技前置和跨多场对局的长期敌方画像；
...
4. 将现有生产承诺账本扩展到建筑施工、维修与可取消的多步计划，并把矿区寿命、路线风险和滚动战损率纳入编制调整；
