...
> 路线边界：本文是 R4/R5 的设计参考，不决定当前任务领取；实时阶段与顺序以 [`AI_DEVELOPMENT_STATE.md`](AI_DEVELOPMENT_STATE.md) 和 [`GAMEPLAY_REWORK_ROADMAP.md`](GAMEPLAY_REWORK_ROADMAP.md) 为准。

> D-017 迁移说明：现有工业/战场主管和敌方八阶段机继续作为规则 AI 技术基础；新切片将把玩家方战场主管实例化为具名将领卡，并把敌方循环改为场景加载时锁定的计划模板与反应表。矿车经济不属于《灰脊矿区》P0。
...
实现状态（2026-08-08）：数据化四档难度、我方四级授权及菜单、权限命令门禁、自主工业/战场基础任务、合法情报目标评分、路线风险、目标切换滞回、攻击追击上限、撤退双阈值、观察后兵种反制、工作流权限摘要和调试决策原因已经落地。全面接管模式已经增加参谋部统一生产仲裁、跨域待付/储备/可用预算账本、玩家生产槽预留、经济恢复优先级、基地告急储备释放、基础兵种画像调整、受阻发展任务自动恢复，以及均衡/经济/固守/进攻四种可直接下达的参谋部方针。自主攻击支持前往最后确认位置搜索和重新接敌。多编队任务图、建筑施工承诺、巡逻/护卫、前后排战术和长期策略画像仍属于后续阶段。
...
当工业主管与战场将领同时处于 `AUTONOMOUS` 时，参谋部是主动生产计划的唯一写入者。主管只能提交各自领域的任务需求，不能各自绕过参谋部填充生产队列。玩家明确下达的发展委托仍由任务本身请求必要生产，以保持 `ASSISTED` 和 `DELEGATED` 的既有语义。

```mermaid
flowchart TD
    A[每次参谋部决策] --> B[读取本方快照、待执行命令和生产队列]
    B --> C[计算待付成本、应急储备和可用预算]
    C --> D{工业自主且矿车承诺少于 2?}
    D -- 是 --> E[只提交一辆矿车生产承诺]
    E --> F[发展任务负责采矿、等待生产和验收]
    D -- 否 --> G{存在可见基地威胁?}
    G -- 是 --> H[进入 BASE_DEFENSE，允许动用 400 储备]
    G -- 否 --> I[保留 400 应急储备]
    H --> J[按前线、侦察、远程缺口选择一项生产]
    I --> J
    J --> K{生产建筑仍有 AI 可用槽?}
    K -- 否 --> L[等待并保留最后一个玩家槽]
    K -- 是 --> M{扣除待付和储备后可负担?}
    M -- 否 --> N[保持资源，不提交低优先级命令]
    M -- 是 --> O[提交一项合法 ProduceUnitCommand]
    F --> P[工作流显示决策与预算]
    L --> P
    N --> P
    O --> P
...
| 字段 | 含义 | 是否已从阵营矿石扣除 |
|---|---|---|
| `pending` | 当前 tick 命令队列中尚未应用的生产成本 | 否 |
| `reserved` | 非紧急状态下不可被常规扩军动用的 400 矿石 | 否 |
| `available` | 当前矿石减去 `pending` 和 `reserved` 后的常规可用额 | 不适用 |

生产承诺按“已存在单位 + 正在生产 + 建筑队列 + 待执行生产命令”计数，防止同一缺口在相邻决策 tick 被重复填充。AI 最多使用每座生产建筑的四个槽位，最后一个槽始终留给玩家直接命令。基地存在当前可见威胁时，参谋部可以把 `reserved` 降为 0，但仍只提交一个确定性的生产承诺。

基础合成编制为 1 辆侦察车、3 辆突击车和 2 辆导弹车。生产顺序先保证至少 2 辆前线突击车，再补侦察、导弹与剩余前线缺口。已观察到的敌方导弹车多于突击车时，突击车目标临时增加 1；观察到的敌方突击车更多时，导弹车目标临时增加 1；单次动态增量有上限，不允许因持续可见目标无限扩军。

发展任务的恢复规则：

1. 玩家显式发展任务若因矿石不足而阻塞，已下达的采矿往返继续运行；余额达到矿车成本后自动恢复并重新提交生产。
2. 主动发展任务不自行生产矿车，只等待参谋部承诺完成，避免工业主管与参谋部重复消费。
3. 指挥中心不可用、队列已满或矿区失效时继续保持明确阻塞状态，不通过直接修改资源或单位绕过规则。
4. 授权降低、玩家接管和直接生产命令仍高于自主计划；AI 不取消玩家队列，也不夺回被接管单位。

#### 8.1.2 玩家直达参谋部方针（已实现）

底部指挥台提供一个“未接管/手动”状态与四种持久接管方针。玩家选择任一接管方针即视为授权“全面接管”：工业主管和战场将领统一切换为 `AUTONOMOUS`，参谋部取消并释放旧的主动战场任务，再从下一次权威决策开始按新方针重建任务。选择“未接管/手动”会结束主动任务并把两名主管恢复为 `ASSISTED`。玩家显式委托、已经支付的生产和被玩家接管的单位不被取消。

| 方针 | 经济目标 | 生产调整 | 战场行为 |
|---|---|---|---|
| 未接管 / 手动 | 玩家决定 | 不主动排产 | 仅执行玩家明确委托 |
| 均衡接管 | 2 辆矿车 | 基础合成编制 | 基地告急时防守，远端可见接敌后反击 |
| 经济优先 | 3 辆矿车 | 先补经济，再补基础合成编制 | 主力保持基地防区，侦察任务继续并行 |
| 基地固守 | 2 辆矿车 | 突击和导弹目标各增加 1 | 主力不追逐远端接触，只攻击防区内威胁 |
| 主动进攻 | 2 辆矿车 | 突击目标增加 2、导弹目标增加 1 | 侦察发现远端目标后主动创建攻击任务 |

方针只改变目标与任务选择，不绕过预算、队列、可见性或单位职责。攻击目标失去视野后，任务前往 `last_seen_position`；到达后搜索 30 tick，期间重新发现则重发合法 `AttackCommand`，超时则结束任务并把单位交还下一轮战场决策。该流程不得读取隐藏目标真值。
